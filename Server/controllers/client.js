...
export const createClient = async (req, res) => {
  upload(req, res, async (err) => {
    try {
      ...
      if (!user || !eventname || !navigation || !agentname || !amount || !prooftype || !sportname || !marketname) {
        return res.status(400).json({
          message: 'Missing required fields',
          details: {
            user: !user ? 'User is required' : '',
            eventname: !eventname ? 'Event name is required' : '',
            navigation: !navigation ? 'Navigation is required' : '',
            agentname: !agentname ? 'Agent name is required' : '',
            amount: !amount ? 'Amount is required' : '',
            prooftype: !prooftype ? 'Proof type is required' : '',
            sportname: !sportname ? 'Sport name is required' : '',
            marketname: !marketname ? 'Market name is required' : '',
          },
        });
      }
      ...
      if (!whitelabelInstance || !proof || !sport || !market) {
        return res.status(400).json({
          message: 'Invalid data provided',
          details: {
            whitelabel: !whitelabelInstance ? 'whitelabel_user not found' : '',
            proof: !proof ? 'Proof type not found' : '',
            sport: !sport ? 'Sport not found' : '',
            market: !market ? 'Market not found' : '',
          },
        });
      }
...
export const updateClient = async (req, res) => {
  upload(req, res, async (err) => {
    try {
      ...
      if (whitelabel_user) {
        const whitelabelInstance = await whitelabel.findOne({ whitelabel_user });
        if (!whitelabelInstance) return res.status(400).json({ message: 'Invalid whitelabel_user' });
        whitelabelId = whitelabelInstance._id;
      }
      if (prooftype) {
        const proof = await ProofType.findOne({ type: prooftype });
        if (!proof) return res.status(400).json({ message: 'Invalid proof type' });
        proofId = proof._id;
      }
      if (sportname) {
        const sport = await Sport.findOne({ sportsName: sportname });
        if (!sport) return res.status(400).json({ message: 'Invalid sport name' });
        sportId = sport._id;
      }
      if (marketname) {
        const market = await Market.findOne({ marketName: marketname });
        if (!market) return res.status(400).json({ message: 'Invalid market name' });
        marketId = market._id;
      }
      ...
      if (!updatedClient) return res.status(404).json({ message: 'Client not found' });
      res.status(200).json(updatedClient); // Fixed syntax error here
    } catch (error) {
      console.error('Update client error:', error);
      res.status(400).json({ message: error.message });
    }
