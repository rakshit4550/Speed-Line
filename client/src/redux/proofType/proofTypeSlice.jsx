...
export const fetchProofByType = createAsyncThunk(
  ...
  async ({ type, user }, { rejectWithValue }) => {
    try {
      ...
      console.error("fetchProofByType error:", error.response?.data);
...
export const fetchAllProofs = createAsyncThunk(
  ...
  async (user, { rejectWithValue }) => {
    try {
      const response = await axios.get(`/prooftype/proofs?user=${user}`);
      console.log("fetchAllProofs response:", response.data);
      return Array.isArray(response.data) ? response.data : [];
    } catch (error) {
      console.error("fetchAllProofs error:", error.response?.data);
      return rejectWithValue(
...
export const updateProof = createAsyncThunk(
  ...
  async ({ type, user, notes }, { rejectWithValue }) => {
    try {
      ...
      console.error("updateProof error:", error.response?.data);
      return rejectWithValue(
...
export const deleteProof = createAsyncThunk(
  ...
  async ({ type, user }, { rejectWithValue }) => {
    try {
      ...
      console.error("deleteProof error:", error.response?.data);
      return rejectWithValue(
...
export const initializeProofs = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      console.error("initializeProofs error:", error.response?.data);
      return rejectWithValue(
...
export const setPreview = createAsyncThunk(
  ...
  async ({ type, user }, { rejectWithValue }) => {
    try {
      ...
      console.error("setPreview error:", error.response?.data);
      return rejectWithValue(
...
// Slice (unchanged except for adding initializeProofs reducer)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllProofs.fulfilled, (state, action) => {
        state.loading = false;
        state.proofs = Array.isArray(action.payload) ? action.payload : [];
      })
