...
//       });
//       setProofMaker(currentClient.proofMaker || '');
//       setSelectedImages([]);
//       setSelectedNavigation2Images([]);
...
//     if (!proofMaker.trim()) {
//       alert('Proof Maker is required before downloading the PDF.');
//       return;
//     }
...
//         if (key !== 'images' && key !== 'navigation2Images' && key !== 'proofContent' && key !== 'whitelabel' && key !== 'group') {
//           if (value !== undefined && value !== '') {
...
//         // Convert base64 back to file if necessary
//         // For simplicity, assume images are already stored in backend
//       });
//       previewData.navigation2Images.forEach((image) => {
//         // Convert base64 back to file if necessary
//       });

//       let action;
//       if (view === 'edit') {
//         action = await dispatch(updateClient({ id: editId, clientData: updatedFormData }));
//       } else {
...
//             <div>૨. 1355*280 અને 5 Bets, ઉબન્ટુ: 1600*330 અને 5 Bets, વિન્ડોઝ: 1270*280 અને 5 Bets</div>
//             <li>Double Bets VOID ના Proof માટે સિસ્ટમ પ્રમાણે નીચેના માપથી SS લો.</li>
...
//         <div>
//           <label className="block text-sm font-medium text-gray-700">
//             <div>Navigation 2 Images (up to 6, Optional)</div>
//             <div>સારા પરિણામો માટે નીચની સૂચનાઓ વાંચો અને અનુસરો.</div>
//             <div>૧. તમારું બ્રાઉઝર ગૂગલ ક્રોમ રાખો.</div>
//             <div>૨. તમારા બ્રાઉઝરનું રેસોલુશન ૧૦૦% રાખો.</div>
...
//                 disabled={!proofMaker.trim() || isDownloading}
...
//                   !proofMaker.trim() || isDownloading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'
//                 }`}
//               >
...
function ClientManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setPreviewData({
        ...
        proofContent,
        whitelabel: { ...matchedWhitelabel, logoBase64 },
        proofMaker: proofMaker || currentClient?.proofMaker || '',
      });
      setView('preview');
    } catch (error) {
      console.error('Submit error:', error);
  ...
  const handleUpdateFromPreview = () => {
    if (!currentClient?._id && view === 'edit') {
      alert('No client ID available for updating.');
      return;
    }
    ...
    // Restore formData from previewData
    setFormData({
    ...
    // Restore proofMaker
    setProofMaker(previewData?.proofMaker || '');

    // Validate whitelabel_user
    const matchedUser = whitelabels.find(
      (wl) => wl.whitelabel_user === previewData?.whitelabel_user
    );
    setIsValidUsername(!!matchedUser);
    setUsernameStatus(matchedUser ? 'User found' : 'User not found');

    // Restore images
    const restoredImages = previewData?.images?.map((img) => ({
      base64: img.path,
      filename: img.filename,
      file: base64ToFile(img.path, img.filename),
    })) || [];
    setSelectedImages(restoredImages);

    // Restore navigation2 images
    const restoredNav2Images = previewData?.navigation2Images?.map((img) => ({
      base64: img.path,
      filename: img.filename,
      file: base64ToFile(img.path, img.filename),
    })) || [];
    setSelectedNavigation2Images(restoredNav2Images);

    // Re-fetch proof content for the prooftype
    if (previewData?.prooftype) {
      dispatch(fetchProofByType(previewData.prooftype));
    }

    // Set editId for updates
    setEditId(currentClient?._id || null);
  };
    ...
    try {
      ...
      previewData.images.forEach((image) => {
        if (image.path.startsWith('data:')) {
          const file = base64ToFile(image.path, image.filename);
      ...
      previewData.navigation2Images.forEach((image) => {
        if (image.path.startsWith('data:')) {
          const file = base64ToFile(image.path, image.filename);
          updatedFormData.append('navigation2Images', file);
        }
      });
  ...
  <div className="container mx-auto px-4 py-8">
    ...
    <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6 space-y-4">
      ...
      <div>
        ...
        {selectedImages.length > 0 && (
          <div className="mt-2">
            <p className="text-sm font-medium text-gray-700">Selected Images ({selectedImages.length}):</p>
      ...
      <div>
        ...
        {selectedNavigation2Images.length > 0 && (
          <div className="mt-2">
            <p className="text-sm font-medium text-gray-700">Selected Navigation 2 Images ({selectedNavigation2Images.length}):</p>
            <div className="flex flex-wrap gap-2 mt-2">
