...
function ClientManager() {
  ...
  const [suggestions, setSuggestions] = useState([]); // Moved to top level
  const [showDropdown, setShowDropdown] = useState(false); // Moved to top level
  const previewRef = useRef(null);

  // Use actual whitelabel users from Redux state
  ...
  const getPreviewHTML = () => {
    ...
    const navigation2ImagesHTML = previewData?.navigation2Images?.length
      ? previewData.navigation2Images
          ...
          .join('')
      : '<p class="text-gray-500">No Navigation 2 images available</p>';
          ...
          <div class="min-h-[842px] mx-auto flex flex-col">
            ...
            <main class="p-5" style="min-height: calc(842px - 100px);">
              ...
              ${previewData?.navigation2 ? `
                <div class="italic font-bold mt-[25px] mx-[24px]">
                  <span class="font-bold text-[16px]">M${previewData.navigation2}</span>
                </div>
                <div class="mt-5 w-full flex flex-wrap gap-2.5 mr-[25px]">
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Whitelabel User</th>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Navigation 2</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Profit & Loss</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Images</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Navigation 2 Images</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {clients.map((client) => (
              <tr key={client._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">{client.navigation2 || ''}</td>
  ...
  const renderForm = () => (
    <div className="container mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Navigation 2 (Optional)</label>
          <input
            type="text"
            name="navigation2"
