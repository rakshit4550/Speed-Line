...
const ProofManagement = () => {
  ...
  // Toggle edit mode or create new proof
  const handleEditClick = useCallback(() => {
    if (currentProof) {
      setEditNotes(currentProof.notes || "");
      setEditMode(true);
    } else if (selectedProofType && selectedUser) {
      setEditNotes("");
      setEditMode(true); // Allow creating new proof
    }
  }, [currentProof, selectedProofType, selectedUser]);

  // Handle notes update or creation
  const handleNotesUpdate = useCallback(() => {
    if (selectedProofType && selectedUser) {
      dispatch(
        updateProofContent({
  ...
  }, [dispatch, selectedProofType, selectedUser, editNotes]);

  // Get the content and notes to display
  const displayContent = currentProof ? currentProof.content : "";
  const displayNotes = currentProof ? currentProof.notes : "";
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Content and Notes Display/Edit Section */}
        {!loading && selectedProofType && (
          <div className="bg-white shadow-md rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">
                {selectedProofType} Details
              </h2>
              {!editMode ? (
                <button
                  ...
                  disabled={loading}
                >
                  {currentProof && currentProof.notes ? (
                    <>
                      <FaEdit className="mr-2" /> Edit Notes
                    </>
                  ) : (
                    <>
                      <FaPlus className="mr-2" /> Add Notes
                    </>
                  )}
                </button>
              ) : (
                <button
                  ...
                  disabled={loading}
                >
                  <FaSave className="mr-2" /> Save Notes
                </button>
              )}
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="prose max-w-none border border-gray-200 rounded-lg p-4 min-h-[100px] bg-gray-50">
                {displayContent || "No default content available"}
              </div>
            </div>
            ...
            <div>
              ...
              {editMode ? (
                ...
                <div className="prose max-w-none border border-gray-200 rounded-lg p-4 min-h-[200px] bg-gray-50">
                  {displayNotes || "No notes added. Click 'Add Notes' to include your notes."}
                </div>
              )}
            </div>
          </div>
